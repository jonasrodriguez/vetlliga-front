...
interface TestModalProps {
  ...
  initialData?: TestEntry | null; // Data for editing, null for adding
}

const TestsModal: React.FC<TestModalProps> = ({ open, onClose, onSave, onDelete, initialData }) => {
  const [formData, setFormData] = useState<TestEntry>(
    initialData || { id: 0, fecha: '', tipo: '', resultado: '', lote: '' }
  );
  ...
  return (
    <ResusableDetailsModal
      open={open}
      onClose={onClose}
      title={formData.id ? 'Editar Test' : 'Nuevo Test'}
      onSave={handleSave}
